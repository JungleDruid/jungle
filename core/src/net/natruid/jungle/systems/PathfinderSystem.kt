...
class PathfinderSystem : BaseSystem() {
    ...
    private class Pathfinder(
        val pathfinderSystem: PathfinderSystem,
        val mTile: ComponentMapper<TileComponent>,
            ...
            while (!frontier.isEmpty) {
                ...
                repeat(if (diagonal) 2 else 1) { i ->
                    if (searchNeighbors(current, i > 0, goal = goal, buildingRoad = false))
                        return pathfinderSystem.extractPath(visited.values, goal, unit, type, maxCost)
                }
            }
            return null
        }
    ...
    private val pool = object : Pool<Pathfinder>(8) {
        override fun newObject(): Pathfinder {
            return Pathfinder(this@PathfinderSystem, mTile, tileSystem)
        }
    }

    @Synchronized
    private fun obtain(): Pathfinder {
        return pool.obtain()
    }

    @Synchronized
    private fun free(pathfinder: Pathfinder) {
